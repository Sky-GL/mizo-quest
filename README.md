...
会話の「通しで聞く」と単語の「1字ずつ読む」は、**1つ言い終えてから次に移ります**
(`speakSequence`)。固定時間で次へ送ると長い文が途中で切れるため、
`onend` を待って繋ぐようにしてあります。再生中の行はハイライトされます。

**読み上げの速さは中身で決まります** (`naturalRate`)。

| 中身 | 速さ | 理由 |
|---|---|---|
| 文(空白・句読点を含む) | 1.0 | ゆっくり読ませると単語がぶつ切りに聞こえて流れなくなる |
| 単語 | 0.9 | |
| A / AW / CH のような1字単位 | 0.7 | 舌の形を追えるように |

会話画面には **🐇ふつう / 🐢ゆっくり** の切り替えがあります。ふつうが既定で、
聞き取れないときだけ落とす想定です(ゆっくりは約1.5倍の時間になり、行間も広がります)。
